...
with open('./config_files/application.cfg') as f:
    cfg.read_file(f)
redis_config = eval(cfg.get('redis', 'redis'))
try:
    r = redis.Redis(**redis_config)
    r.ping()
    print("连接redis成功，可以提供进度查询功能")
except Exception as e:
    print(e)
    print('WARNING: 无法提供进度查询功能')
    r = R()
...
def func():
    if isinstance(r, R):
        pass
    elif not r.get('progress'):  # 第一次调用时，redis中可能不存在'progress'键
        pass
...
def query_progress():
    return jsonify({'state': 1, 'msg': round(float(str(r.get('progress'), 'utf-8')), 2)})
...
def main_fuse():
    ...
    if root_res_df is None:
        ...
        for i in range(len(root_res_df)):
            r.set('progress', (i + 1)/len(root_res_df))
