# -*- coding: utf-8 -*-
"""
@Time   : 2020/6/17 1:53 下午
@Author : sunjiawei
@E-mail : j.w.sun1992@gmail.com

Check if all settings are valid when starting the service.
"""
    ...
    if cms:
        for i in range(len(cms_entities)):
            if not f(eval(cms_pros[i])):
                raise NotEnoughProperties(f"At least one property needed for {cms_entities[i]} of cms, found zero")
    if pms:
        for i in range(len(pms_entities)):
            if not f(eval(pms_pros[i])):
                raise NotEnoughProperties(f"At least one property needed for {pms_entities[i]} of pms, found zero")
    if gis:
        for i in range(len(gis_entities)):
            if not f(eval(gis_pros[i])):
                raise NotEnoughProperties(f"At least one property needed for {gis_entities[i]} of gis, found zero")

    # Are properties' detail of each entity from each system the same?
    if list(map(len, (cms, pms, gis))).count(0) == 0:
        for i in range(len(cms_entities)):
            if not len(eval(cms_pros[i])['TEXT']) == len(eval(pms_pros[i])['TEXT']) == len(eval(gis_pros[i])['TEXT']):
                raise PropertiesNotMatch(f"'TEXT' properties doesn't match among three systems(property no. {i})")
            if not len(eval(cms_pros[i])['ENUM']) == len(eval(pms_pros[i])['ENUM']) == len(eval(gis_pros[i])['ENUM']):
